...
export const downloadInvoicePDF = async (info: BankTransferInfo) => {
  try {
    const response = await fetch(
      `${window.location.origin}/api/invoice/generate`,
      {
        method: "POST",
        headers: {
...
export const downloadInvoicesAsZip = async (
  invoicesData: BankTransferInfo[]
) => {
  try {
    const response = await fetch(
      `${window.location.origin}/api/invoice/generate-zip`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
